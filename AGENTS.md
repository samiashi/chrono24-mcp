...
- Keep requests serialized and slow (~3.5s page-scrape spacing; same-origin JSON APIs use the shorter JSON_REQUEST_DELAY_MS since the site's own pages fire them freely). Never add concurrency or parallel fetches - Cloudflare escalates and IP-blocks. The Fetcher enforces this with an internal promise-queue mutex (`createSerializer`); every new fetch path must go through `enqueue`.
...
- Currency is assigned by Chrono24 from server geolocation and CANNOT be configured: the `currencyId` URL param is ignored even on a fresh profile's first request (probed 2026-09 with CURRENCY_ID=AED - prices stayed USD), and the preference store (`c24-data` cookie) is an opaque numeric-keyed blob. Responses therefore report the currency DETECTED from prices (`detectCurrency`), with `CURRENCY_ID` only as fallback text.
