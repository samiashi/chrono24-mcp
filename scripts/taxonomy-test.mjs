...
try {
  ...
  console.error(
    `list_brands() -> ${a.count} brands, sample: ${a.brands
      .slice(0, 3)
      .map((x) => `${x.name}=${x.id}`)
      .join(", ")}`,
  );
  ...
  const precise = await client.callTool({
    ...
    arguments: {
      manufacturerIds: m.brand.id,
      models: m.models.find((x) => x.slug === "submariner")?.modelId,
      priceTo: 15000,
      limit: 5,
    },
  });
  ...
  console.error(
    `precise search (Rolex+Submariner, <=$15k) -> totalCount=${p.totalCount}, showing ${p.count}:`,
  );
