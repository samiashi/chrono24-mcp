...
export const numFrom = (name: string, v: string | undefined, def: number): number => {
  if (v === undefined) return def;
  const n = Number(v);
  if (!Number.isFinite(n)) {
    console.error(`[config] ignoring invalid ${name}="${v}", using default ${def}`);
    return def;
  }
  return n;
};
...
export const config = {
  ...
  navigationTimeoutMs: numFrom("NAVIGATION_TIMEOUT_MS", process.env.NAVIGATION_TIMEOUT_MS, 45000),
  challengeTimeoutMs: numFrom("CHALLENGE_TIMEOUT_MS", process.env.CHALLENGE_TIMEOUT_MS, 45000),
  ...
  maxBatch: numFrom("MAX_BATCH", process.env.MAX_BATCH, 10),
